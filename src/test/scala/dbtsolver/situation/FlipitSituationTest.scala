...
import dbtsolver.situation.{FlipitSituation => FS}

@Test
class FlipitSituationTest {

  @Test
  def testEmptySituation() {

    def a = FS(null, Array[Array[Boolean]]())
    assertEquals("[]", a.toString())
//    assertFalse(a.isWinning())

//    def b = FS(null, Array[Int]())
...
/*
  ...
  def testTwoBarSituation() {
    ...
    def a = FS(null, Array[Int](1, 2))
    assertEquals("[1, 2]", a.toString())
    assertFalse(a.isWinning())

    def b = FS(null, Array[Int](1, 2))
  ...
  def testThreeBarSituation() {
    ...
    def a = new FS(null, Array[Int](1, 2, 3))
    assertFalse(a.isWinning())

    def b = new FS(null, Array[Int](3, 2, 1))
    assertFalse(b.isWinning())

    def c = new FS(null, Array[Int](2, 2, 2))
    assertTrue(c.isWinning())

    def d = FS(null, Array[Int](2, 2, 2))
  ...
  @Test
  def testAdjacency() {
    def s0 = FS(null, Array(2, 3))
    val s1 = FS(s0, Array(3, 3))
    val s2 = FS(s0, Array(1, 3))
    val s3 = FS(s0, Array(2, 4))
    val s4 = FS(s0, Array(2, 2))
